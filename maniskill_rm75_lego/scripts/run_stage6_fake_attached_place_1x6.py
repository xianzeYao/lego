...
from maniskill_rm75_lego.apex_mr_reference import APEX_PLACE_BRICK_OFFSET, APEX_TWIST_DEG
...
def parse_args():
    ...
    parser.add_argument("--include-pick", action="store_true")
    ...
    parser.add_argument("--place-press-depth", type=float, default=0.003)
...
def main() -> int:
    ...
    pick_twist_contact = pick_down_contact @ rot_y_matrix(args.pick_twist_deg)
    ...
    if args.include_pick:
        # Attach at twist/contact, before lifting. Attaching after lift records an artificial gap.
        contact_t_brick = invert_transform(pick_twist_contact) @ initial_brick_mat
    ...
    place_twist_contact = place_press_contact @ rot_y_matrix(args.place_twist_deg)
    place_up_contact = shifted_pose(place_twist_contact, [0.0, 0.0, args.place_up_height])

    contact_poses = {
        "pre_pick": pick_contact @ translation_matrix([0.0, 0.0, -args.pre_height]),
    ...
    frame_markers = {}
    for name in ["pick_attach", "transfer", "pre_place", "drop_up", "place_down", "place_twist", "place_up"]:
    ...
    if args.include_pick:
        ...
        for deg in np.linspace(0.0, args.pick_twist_deg, max(1, args.twist_ik_steps) + 1)[1:]:
            twist_pose = pick_down_contact @ rot_y_matrix(float(deg))
    ...
    if args.include_pick:
        print_names = ["pre_pick", "pick_down", "pick_twist", "pick_attach"] + print_names
    ...
    if args.include_pick:
        ...
        attached = True
        update_fake_attach()
        print("[attach] fake attach at pick_twist contact frame")
    ...
    print("[release] snap fake-attached brick to target grid pose")
