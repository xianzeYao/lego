...
def stability_score_with_support(lego_structure, support_x, support_y, support_z, support_ori, lego_library, config):
    ############### Setup ###############
    ...
    # Setup Support
    lego_structure = copy.deepcopy(lego_structure)
    world_grid = construct_world_grid(lego_structure, world_dim, brick_library) 
    b100_seq = 1
    for z in range(0, support_z+1):
        if(world_grid[support_x, support_y, z] == 0):
            lego_structure[str(len(lego_structure)+1)] = {
                "x":support_x,
                "y":support_y,
                "z":z+1,
                "brick_id":90,
    ...
    if(visualize):
        ax.voxels(world_grid, facecolors=heatmap_color, edgecolor='w')
        plt.show()
